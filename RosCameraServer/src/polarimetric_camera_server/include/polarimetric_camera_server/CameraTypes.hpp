...
/**
 * @brief CameraConfig struct: It is used to compactely send the cameras
 *   configuration, defined by the launch file.
 *
 * @arg camName: String with the user-defined camera name. It can be empty
 *   only when a single camera is used.
 * @arg camDriver: Camera driver to be used.
 * @arg isMaster: Boolean. If true, the camera runs independently of a triggering
 *   signal. If False, the camera will return images ONLY if an external trigger
 *   signal have been provided on its back connector.
*/
typedef struct {
    std::string camName;
    std::string camDriver;
    bool isMaster;
} CameraConfig;
