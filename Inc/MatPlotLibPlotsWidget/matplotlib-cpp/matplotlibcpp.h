#ifndef __MATPLOTLIBCPP_HPP__
#define __MATPLOTLIBCPP_HPP__

// Python headers must be included before any system headers, since
// they define _POSIX_C_SOURCE

/// Without these push_macro and pop_macro code, Qt and Python3 cannot exist together.
// Taken from https://stackoverflow.com/a/49359288
...
struct _interpreter {
    ...
    PyObject *s_python_function_get_fignums;
...
#  endif
    ...
    _interpreter() {
        ...
        s_python_function_get_fignums = safe_import(pymod, "get_fignums");
...
inline long get_num_figs()
{
    detail::_interpreter::get();

    PyObject* res = PyObject_CallObject(detail::_interpreter::get().s_python_function_get_fignums, detail::_interpreter::get().s_python_empty_tuple);
    if (PyErr_Occurred()) {
        PyErr_PrintEx(0);
...
inline long gcf_number()
{
    detail::_interpreter::get();

    PyObject *gcf =
    PyObject_CallObject(detail::_interpreter::get().s_python_function_gcf,
      detail::_interpreter::get().s_python_empty_tuple);
    if (!gcf) throw std::runtime_error("Call to gcf() failed.");
    Py_INCREF(gcf);

    PyObject *number = PyObject_GetAttrString(gcf, "number");
    if (!number) throw std::runtime_error("Attribute number not found.");
    Py_INCREF(number);

    long img_number = static_cast<long>(PyLong_AsLong(number));
...
inline void draw_idle()
{
    detail::_interpreter::get();

    PyObject *gcf = PyObject_CallObject(detail::_interpreter::get().s_python_function_gcf, detail::_interpreter::get().s_python_empty_tuple);
    if (!gcf) throw std::runtime_error("Call to gcf() failed.");
    Py_INCREF(gcf);

    PyObject *canvas = PyObject_GetAttrString(gcf, "canvas");
    if (!canvas) throw std::runtime_error("Attribute canvas not found.");
    Py_INCREF(canvas);

    PyObject *drawIdle = PyObject_GetAttrString(canvas, "draw_idle");
    if (!drawIdle) throw std::runtime_error("Attribute draw_idle not found.");
    Py_INCREF(drawIdle);

    PyObject* res = PyObject_CallObject(drawIdle, detail::_interpreter::get().s_python_empty_tuple);
    if(!res) throw std::runtime_error("Call to draw_idle() failed.");
...
template<typename Numeric>
inline void start_event_loop(Numeric interval)
{
    detail::_interpreter::get();
    ...
    PyObject *eventLoop = PyObject_GetAttrString(canvas, "start_event_loop");
    if (!eventLoop) throw std::runtime_error("Attribute start_event_loop not found.");
    Py_INCREF(eventLoop);

    PyObject* args = PyTuple_New(1);
    PyTuple_SetItem(args, 0, PyFloat_FromDouble(interval));

    PyObject* res = PyObject_CallObject(eventLoop, args);
    if(!res) throw std::runtime_error("Call to start_event_loop() failed.");

    Py_XDECREF(gcf);
    Py_XDECREF(canvas);
    Py_XDECREF(eventLoop);
    Py_XDECREF(args);
}
...
inline void close()
{
    ...
    PyObject* all_str = PyString_FromString("all");
    PyObject* args = PyTuple_New(1);
    PyTuple_SetItem(args, 0, all_str);
